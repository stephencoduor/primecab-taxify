...
export function LocationDrop() {
  ...
  useEffect(() => {
    ...
    if (destination) convertToCoords(destination, setDestinationCoords);
  ...
  const fetchAddressSuggestions = async (input: string) => {
    if (input.length >= 3) {
      const apiUrl = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${input}&location=${latitude},${longitude}&radius=50000&key=${Google_Map_Key}`;
                    ...
                    >
                      ...
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', width: '90%' }}>
                        ...
                        <View style={{ justifyContent: 'center', alignItems: 'center' }}>
                          ...
                          <Text style={{ fontFamily: appFonts.medium, color: appColors.primary }}>{taxidoSettingData?.taxido_values?.ride?.distance_unit}</Text>
                        </View>
                      </View>
                    </TouchableOpacity>
                  ))
